...
\lstdefinelanguage{Isabelle}{
  ...
  keywords=[1]{definition, if, then, else, let, in, case, of},
  % Custom keyword styles
  keywordstyle=[1]\color{isa_blue},
  keywords=[2]{where},
  keywordstyle=[2]\color{isa_green},
  keywords=[3]{RETURN, WHILEIT, SPEC, ASSERT},
...
For the first \texttt{SPEC}, we define $\$_{\texttt{pick\_splitter}}$ and for the second \texttt{SPEC}, we define $\$_{\texttt{split\_and\_update}}$. We must not split the latter into two separate costs for the moment since both operations will be performed on the fly, see the proof of lemma \ref{lem:bound} for more details.
...
\end{lstlisting}
Informally again, the complexity lies in $\$_{\texttt{split\_and\_update}}$ and the other costs will be constant eventually.

We know from lemma \ref{lem:bound} that in the end, if the loop body is executed for $\mathcal{A}$ and if the current partition and set of splitters is $(P, L)$ and some splitter $(a, C)$ is picked in the second \texttt{ASSERT}, then the implementation of the program should use at most \texttt{card($\Sigma$ $\mathcal{A}$) * card (($\lambda$ q. $\delta^{-1}$(q, a)) \textasciigrave\ C)} (with some constant factor) coins of some currency.
