...
We know from lemma \ref{lem:bound} that if \texttt{abstract\_f} is called for $\mathcal{A}$ and if the current partition and set of splitters is $(P, L)$ and some splitter $(a, C)$ is picked in the second \texttt{ASSERT}, then the program should use at most \texttt{card($\Sigma$ $\mathcal{A}$) * card (($\lambda$ q. $\delta^{-1}$(q, a)) \textasciigrave\ C)} coins of some currency (modeling the constant of proportionality).
...
For the first \texttt{SPEC}, we define $\$_{\texttt{pick\_splitter}}$ and for the second \texttt{SPEC}, we define $\$_{\texttt{update\_splitters}}$ and $\$_{\texttt{split}}$.
