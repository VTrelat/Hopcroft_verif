...
\begin{definition}
    For a splitter $s \in \Sigma \times \mathcal{P}$, we define its size $\| s \|$ as follows. If $s = (a, B)$, then $\| s \| := |B|$.
\end{definition}
...
\begin{proof}
    ...
    \textit{During an iteration, the chosen splitter $s$ will either be removed from the set of splitters or its size will be reduced by at least $\frac{\| s\|}{2}$ after the iteration.}
