...
locale array_block1 =
  fixes static_err :: 'err
    and mem_err :: 'err
    and vload :: "'vaddr::this_addr \<Rightarrow> ('val,_,'c::monoid_add,'val,'err) M"
    and vstore :: "'val \<Rightarrow> 'vaddr \<Rightarrow> (unit,_,'c::monoid_add,'val,'err) M"
    and vgep :: "'vaddr \<Rightarrow> 'vidx \<Rightarrow> ('vaddr,_,'c::monoid_add,'val,'err) M"
begin
  definition load :: "'vaddr baddr \<Rightarrow> ('val,_,'c::monoid_add,'val block,'err) M" where
    "load \<equiv> \<lambda>BADDR i va \<Rightarrow> fcheck mem_err (i\<ge>0) \<then> zoom (lift_lens mem_err (idx\<^sub>L (nat i))) (vload va)"

  definition store :: "'val \<Rightarrow> 'vaddr baddr \<Rightarrow> (unit,_,'c::monoid_add,'val block,'err) M" where
  ...
  definition check_addr :: "'vaddr baddr \<Rightarrow> (unit,_,'c::monoid_add,'val block,'err) M" where
  ...
  definition checked_idx_baddr :: "'vaddr baddr \<Rightarrow> int \<Rightarrow> ('vaddr baddr, _,'c::monoid_add, 'val list, 'err) M" where 
  ...
  definition checked_gep_addr :: "'vaddr baddr \<Rightarrow> 'vidx \<Rightarrow> ('vaddr baddr, _,'c::monoid_add, 'val list, 'err) M"
    where
