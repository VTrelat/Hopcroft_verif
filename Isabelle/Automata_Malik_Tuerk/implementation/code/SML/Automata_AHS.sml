...
signature Sigma_param = sig
   ...
   *)
   val Sigma_dsarg : Sigma HOL.equal * Sigma HashCode.hashable
...
structure string_Sigma_param : Sigma_param = struct
  ...
  *)
  ...
  fun int_to_Sigma n = Char.toString (chr (n+96))
end
...
functor NFA (S : Sigma_param) : sig
  ...
  *)
  ...
  val nfa_is_deterministic : NFA -> bool
...
(* For RBT Implementation 
type NFA = (int, unit) RBT.rbt *
   ((S.Sigma, unit) RBT.rbt *
   ((int * S.Sigma, (int, unit) RBT.rbt) RBT.rbt *
   ((int, unit) RBT.rbt * (int, unit) RBT.rbt)))
*)


type NFA =
      (((IntInf.int, unit) ArrayHashMap.hashmap *
         ((S.Sigma, unit) ArrayHashMap.hashmap *
           ((IntInf.int, (S.Sigma, (IntInf.int, unit) ArrayHashMap.hashmap) ArrayHashMap.hashmap)
              ArrayHashMap.hashmap *
             ((IntInf.int, unit) ArrayHashMap.hashmap *
               (IntInf.int, unit) ArrayHashMap.hashmap)))),
        ((IntInf.int, unit) ArrayHashMap.hashmap *
          ((S.Sigma, unit) ArrayHashMap.hashmap *
            ((IntInf.int, (S.Sigma, IntInf.int) ArrayHashMap.hashmap) ArrayHashMap.hashmap *
              ((IntInf.int, unit) ArrayHashMap.hashmap *
                (IntInf.int, unit) ArrayHashMap.hashmap)))))
        NFAByLTS.automaton;      

val nFA_states_nat = {states_enumerate = (fn i => i)}:(int NFA.nFA_states)
val state_args = (Arith.equal_nat, HashCode.hashable_nat, nFA_states_nat)
val simple_state_args = (HashCode.hashable_nat, nFA_states_nat)


fun nfa_construct (Q, A, D, I, F) = 
   AHS_NFAImpl.ahs_nfa_construct state_args S.Sigma_dsarg
    (Q, (A, (map (fn (a, b,c) => (a, (b,c))) D, (I, F))));

fun dfa_construct (Q, A, D, I, F) = 
   AHS_NFAImpl.ahs_dfa_construct state_args S.Sigma_dsarg
    (Q, (A, (map (fn (a, b,c) => (a, (b,c))) D, (I, F))));

fun nfa_destruct AA =
   let val (Q, (A, (D, (I, F)))) = 
    AHS_NFAImpl.ahs_nfa_destruct state_args (#2 S.Sigma_dsarg) AA in
   (Q, A, map (fn (a, (b,c)) => (a, b,c)) D, I, F) end;

fun nfa_destruct_simple AA =
   let val (Q, (A, (D, (I, F)))) = 
    AHS_NFAImpl.ahs_nfa_destruct_simple simple_state_args (#2 S.Sigma_dsarg) AA in
   (Q, A, map (fn (a, (b,c)) => (a, b,c)) D, I, F) end;
...
(* Basic operations *)
val nfa_accept_base = ahs_nfa_accept state_args S.Sigma_dsarg
fun nfa_accept A w = nfa_accept_base A (S.Sigma_Word_create w)

val nfa_is_deterministic = ahs_nfa_is_deterministic state_args S.Sigma_dsarg
val nfa_rename_labels = ahs_nfa_rename_labels S.Sigma_dsarg state_args
val nfa_normalise = ahs_nfa_normalise state_args S.Sigma_dsarg
val nfa_complement = ahs_nfa_complement state_args
val nfa_bool_comb = ahs_nfa_bool_comb state_args S.Sigma_dsarg
val nfa_product = nfa_bool_comb (fn b1 => fn b2 => b1 andalso b2)
val nfa_determinise = ahs_nfa_determinise state_args S.Sigma_dsarg
val nfa_reverse = ahs_nfa_reverse state_args S.Sigma_dsarg
val nfa_Brzozowski = ahs_nfa_Brzozowski state_args S.Sigma_dsarg
val nfa_Hopcroft = ahs_nfa_Hopcroft state_args S.Sigma_dsarg
val nfa_Hopcroft_NFA = ahs_nfa_Hopcroft_NFA state_args S.Sigma_dsarg


(* Statistic operations *)
val nfa_states_no = ahs_nfa_states_no simple_state_args
val nfa_labels_no = ahs_nfa_labels_no (#2 S.Sigma_dsarg)
val nfa_trans_no = (ahs_nfa_trans_no (#1 simple_state_args) 
  (#2 S.Sigma_dsarg) 
  (#1 simple_state_args) (#1 simple_state_args) (#2 S.Sigma_dsarg)):NFA -> int
val nfa_final_no = ahs_nfa_final_no simple_state_args
val nfa_initial_no = ahs_nfa_initial_no simple_state_args
...
structure int_Sigma_param : Sigma_param = struct
  ...
  val Sigma_dsarg = (Arith.equal_nat, HashCode.hashable_nat);
...
structure bool_list_Sigma_param : Sigma_param = struct
  ...
  *)
  ...
  val Sigma_dsarg = (equal_list, HashCode.hashable_list HashCode.hashable_bool);
...
fun pf_get_free_vars_number (Presburger_Automata.Le (cs, nn)) = length cs
  ...
  fun pf_to_DFA p = AHS_Presburger_Impl.ahs_pres_pf_to_nfa (pf_get_free_vars_number p) p
  fun pf_to_dfa p = Presburger_Automata.dfa_of_pf (pf_get_free_vars_number p) p
  val eval_pf_DFA = AHS_Presburger_Impl.ahs_eval_pf
  val eval_pf_dfa = AHS_Presburger_Impl.eval_pf_dfa
