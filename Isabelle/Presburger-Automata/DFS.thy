...
lemma succss_closed_dfs':
  "invariant ys \<Longrightarrow> list_all is_node xs \<Longrightarrow>
   succss (set_of ys) \<subseteq> set xs \<union> set_of ys \<Longrightarrow> succss (set_of (dfs ys xs)) \<subseteq> set_of (dfs ys xs)"
...
proof -
  ...
  also from succss_closed_dfs xs have "\<dots> = set_of (dfs empt xs)"
    apply (unfold reachable_def)
